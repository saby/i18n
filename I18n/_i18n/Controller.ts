...
export interface IConfigController {
    ...
    availableContexts?: Record<string, IModule>;
    loader?: Loader;
}
...
const STATE_LANG_REGION: Record<regionCode, langCode> = {
    RU: 'ru',
    KZ: 'kk',
    UZ: 'uz',
    TM: 'tk',
...
class Controller implements IController {
    ...
    protected availableContexts: Record<string, IModule>;
    ...
    get stateLanguage(): langCode {
        return STATE_LANG_REGION[this.currentCountry];
    }
    ...
    setLang(code: langCode | string, expires: number = EXPIRES_COOKIES) {
        const languageCode = this._normalizeCode(code);
        const langCookie = Controller.getCookie('lang');

        if (this._isSupportedLang(languageCode) && langCookie !== languageCode) {
            Controller.setCookie('lang', languageCode, expires);
        }
    }
    ...
    addLang(code: langCode, config: ILangConfig, isAvailable: boolean = true) {
        ...
        // TODO По хороше похоже надо делать уже синхронные рельсы для сервера
        if (config) {
            this.loader.history.languages[code] = `I18n/locales/${code}`;
        }
    }
    ...
    addRegion(code: regionCode, config: IRegionConfig, isAvailable: boolean = true) {
        ...
        // TODO По хороше похоже надо делать уже синхронные рельсы для сервера
        if (config) {
            this.loader.history.regions[
                code
            ] = `LocalizationConfigs/localization_configs/region/${code}.json`;
        }
    }
     ...
     * @param version Версия cdn модуля. ModuleName/%version%/lang.
     */
    ...
    protected initStores(): void {
        ...
        this.languagesStore = new Store<langCode, ILangConfig>(
            // TODO По хороше похоже надо делать уже синхронные рельсы для сервера
            constants.isServerSide ? [] : this.requiredLanguages,
        ...
        this.regionStore = new Store<regionCode, IRegionConfig>(
            // TODO По хороше похоже надо делать уже синхронные рельсы для сервера
            constants.isServerSide ? [] : this.requiredRegions,
    ...
    static setCookie(name: string, value: string, expires: number = EXPIRES_COOKIES): void {
        ...
        // TODO нужно описать типы для process, поскольку мы на СП process переопределяем
        // а в нодовских типах не существует req на process.domain
        // @ts-ignore
        if (typeof process === 'object' && process?.domain?.req?.cookies) {
            // @ts-ignore
            process.domain.req.cookies[name] = value;
        }
    }
