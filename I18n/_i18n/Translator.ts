...
class Translator implements ITranslator {
    ...
    translateKey(key: string, context?: string, pluralNumber?: number): string {
        ...
        if (this.dictionaries.hasOwnProperty(code)) {
            if (pluralNumber !== undefined) {
                const pluralForms = this.getTranslatedKey(PLURAL_PREFIX + key, context, code);

                if (pluralForms) {
                    translatedKey = this._translatePlural(pluralForms, pluralNumber, currentLocale);
                }
            } else {
                translatedKey = this.getTranslatedKey(key, context, code);
            }
        }
    ...
    protected getTranslatedKey(key: string, context: string, code: langCode): string {
