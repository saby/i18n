 ...
 */
export function getClassifierByRegion(ISORegionCode: regionCode): string {
    if (REGION_AND_CLASSIFIER.hasOwnProperty(ISORegionCode)) {
        return REGION_AND_CLASSIFIER[ISORegionCode];
    }

    throw new Error(`Classifier code not exists for region code ${ISORegionCode}`);
}
